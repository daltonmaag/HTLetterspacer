...
class HTLetterspacerLib(object):
    ...
    def setSpace(self, layer, referenceLayer):
        ...
        # tabVersion
        if (
            ".tosf" in layer.parent.name
            or ".tf" in layer.parent.name
            or self.tabVersion
        ):
            if self.width:
        ...
        try:
            ...
            # both sidebearings with metric keys
                ...
                self.output += (
                    "Glyph "
                    + layer.parent.name
                    + " has metric keys. Spacing not set.\n"
                )
            # if it is tabular
            ...
            # if it is fraction / silly condition
                ...
                self.output += (
                    "Glyph "
                    + layer.parent.name
                    + ": should be checked and done manually.\n"
                )
            # if not...
            else:
...
class HTLetterspacerScript(object):
    ...
    def window(self):
        ...
        self.w.text_4b = vanilla.TextBox(
            (120, 50, 50, 14), self.engine.paramArea, sizeStyle="small"
        )
        self.w.text_5 = vanilla.TextBox((15, 75, 100, 14), "Depth", sizeStyle="small")
        self.w.text_5b = vanilla.TextBox(
            (120, 75, 50, 14), self.engine.paramDepth, sizeStyle="small"
        )
        self.w.text_6 = vanilla.TextBox(
            (15, 100, 100, 14), "Overshoot", sizeStyle="small"
        )
        self.w.text_6b = vanilla.TextBox(
            (120, 100, 50, 14), self.engine.paramOver, sizeStyle="small"
        )
        self.w.LSB = vanilla.CheckBox(
            (15, 15, 40, 18),
            "LSB",
            value=True,
            sizeStyle="small",
        ...
        self.w.width = vanilla.EditText(
            (170, 15, 40, 18), widthAvg(self.mySelection), sizeStyle="small"
        )
    ...
    def setG(self, layer):
        ...
        if self.font.glyphs[self.engine.reference]:
            self.referenceLayer = self.font.glyphs[self.engine.reference].layers[
                self.layerID
            ]
            if len(self.referenceLayer.paths) < 1:
                self.output += (
                    "WARNING: The reference glyph declared ("
                    + self.engine.reference
                    + ") doesn't have contours. Glyph "
                    + self.layer.parent.name
                    + " was spaced uses its own vertical range.\n"
                )
    ...
    def spaceMain(self):
        ...
        if len(self.mySelection) < 2 and drawAreas == True and lpolygon is not None:
            self.engine.createAreasGlyph(
                self.font, self.mySelection[0], [lpolygon, rpolygon]
            )
    ...
    def copyParameters(self, sender):
        ...
        if not self.setClipboard(copyText):
            GlyphsApp.Message(
                "Clipboard Error",
                "An error occurred: Could not copy the values into the clipboard. Please check Macro Window for details.",
                OKButton=None,
            )
